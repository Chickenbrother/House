    ...
    return (
      <View style={{backgroundColor:'#fff',flex:1}}>
          <View style={{height:42,backgroundColor:'#08BBF9',flexDirection:'row'}}>
              <TouchableOpacity onPress={this.back.bind(this)} style={{marginLeft:10,justifyContent:'center',width:24,height:40,marginTop:10}}>
                 <Image
              ...
              <View style={{flex:1,alignItems:'center',justifyContent:'center'}}>
                 <Text style={{fontSize:18,color:'white',alignSelf:'center',marginTop:22}}>房产登记预约</Text>
           ...
           <ScrollView showsVerticalScrollIndicator={true}
            ...
            <Text style={{width:Width,backgroundColor:'#E7E6E6',marginTop:10,fontSize:14,alignItems:'flex-start',justifyContent:'flex-start'}}>平海大厦({date})</Text>
               <View>
                 <Text style={styles.label1}>办理业务</Text>
                 <PickerIOS
                      itemStyle={styles.input1}
                      mode={'dropdown'}
                      selectedValue={this.state.banlyw}
                      onValueChange={value => this.setState({banlyw: value})}>
                        {this.state.dataSource.map((item,i) =>
                         <PickerIOS.Item style={{fontSize:12}} label={item.ywlb} value={item.ywlbId} key={i}/> )}
                    </PickerIOS>
                 ...
                 <View>
                   <Text style={styles.label1}>房屋套数</Text>
                   <PickerIOS
                      mode={'dropdown'}
                      itemStyle={styles.input1}
                      selectedValue={this.state.taoshu}
                      onValueChange={(value) => this.setState({taoshu: value})}>
                      <PickerIOS.Item label="1套" value="1" style={{fontSize:12}}/>
                      <PickerIOS.Item label="2套" value="2" style={{fontSize:12}}/>
                      <PickerIOS.Item label="3套" value="3" style={{fontSize:12}}/>
                    </PickerIOS>
                 </View>

                 <View>
                   <Text style={styles.label1}>预约时间</Text>
                   <PickerIOS
                        itemStyle={styles.input1}
                        mode={'dropdown'}
                        ...
                        onValueChange={value => this.setState({yuysj: value})}>
                          {this.state.time.map((item,i) =>
                           <PickerIOS.Item style={{fontSize:16}} label={item.sjd + item.slsj} value={item.chbId} key={i}/> )}
                      </PickerIOS>
                 </View>
